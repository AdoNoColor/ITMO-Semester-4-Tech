...
public class BanksTests {
    ...
    public void CreatingClientsAndAccount() {
        Bank sberbank = CentralBank.getCentralBank().registerBank("Sberbank", 5000, 0.3, 0.1, 50000, 300000);
    ...
    public void Transactions() {
        Bank tinkoff = CentralBank.getCentralBank().registerBank("Tinkoff", 5000, 0.3, 0.1, 50000, 300000);
        Bank sberbank = CentralBank.getCentralBank().registerBank("Sberbank", 5000, 0.3, 0.1, 50000, 300000);
        ...
        CentralBank.getCentralBank().topUpBalance(maxDebitAccount, 300000);
        Assertions.assertEquals(300000, maxDebitAccount.Balance);

        CentralBank.getCentralBank().withdrawMoney(maxDebitAccount, 5000);
        Assertions.assertEquals(295000, maxDebitAccount.Balance);

        Assertions.assertThrows(BanksException.class, () -> CentralBank.getCentralBank().withdrawMoney(maxCreditAccount, 5000));
        Assertions.assertThrows(BanksException.class, () -> CentralBank.getCentralBank().withdrawMoney(maxDepositAccount, 5000));

        CentralBank.getCentralBank().transferMoney(maxDebitAccount, joshDebitAccount, 3000);
    ...
    public void CancellingTransaction() {
        ...
        Operation topUpOperation = CentralBank.getCentralBank().topUpBalance(maxDebitAccount, 300000);
        ...
        CentralBank.getCentralBank().topUpBalance(maxDebitAccount, 300000);
        Operation withdrawOperation = CentralBank.getCentralBank().withdrawMoney(maxDebitAccount, 100);
        CentralBank.getCentralBank().cancelOperation(withdrawOperation);
        Assertions.assertThrows(BanksException.class, () -> CentralBank.getCentralBank().cancelOperation(topUpOperation));

        Operation transferOperation = CentralBank.getCentralBank().transferMoney(maxDebitAccount, joshDebitAccount, 100);
